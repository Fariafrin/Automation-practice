...
test("OrangeHRM Login Test", async ({ page }) => {
    ...
    const employeeName = "Sheila Rosamond";
    await page
        .locator(`tr:has-text("${employeeName}").oxd-icon .bi-pencil-fill`)
        .click();

    // await expect.soft(page).toHaveURL(/.*PersonalDetails/);
    // const personaldetails = page.locator("h6:has-text('Personal Details')");
    // await expect.soft(personaldetails).toBeVisible();
